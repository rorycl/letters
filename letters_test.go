...
func testEmailHeadersFromFile(t *testing.T, fp string, expectedEmailHeaders Headers) {
	...
	if got, want := parsedEmailHeaders, expectedEmailHeaders; !cmp.Equal(want, got) {
...
func testEmailFromFile(t *testing.T, fp string, expectedEmail Email) {
	...
	if diff := cmp.Diff(want,
		got,
		cmp.AllowUnexported(AttachedFile{}),
		cmp.AllowUnexported(InlineFile{}),
		cmp.AllowUnexported(fileData{}),
		cmpopts.IgnoreFields(fileData{}, "writerFunc"),
	); diff != "" {
